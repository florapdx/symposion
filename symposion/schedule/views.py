...
from symposion.schedule.forms import SlotEditForm, PresentationForm
...
@login_required
def schedule_presentation_edit(request, pk):

    presentation = get_object_or_404(Presentation, pk=pk)

    if not presentation and not request.user.is_staff:
        raise Http404()
    ...
    return render(request, "schedule/presentation_edit.html", {
        "form": form,
        })
